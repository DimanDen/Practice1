...
std::string StringRep(int IndexLeft, int IndexRight, std::string str, std::string strReplace) 
{
	...
	{
		if ( i < static_cast<size_t>(IndexLeft) || i > static_cast<size_t>(IndexRight)) //≈сли мы не дошли до того места, когда пора замен€ть, то просто копируем входную строку во вспомогательную
		{
			StrHelpRep += str[i];
		}
		else
